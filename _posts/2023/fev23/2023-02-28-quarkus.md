...
<p>This post used this book as a refence.</p>

<p>
  <center>
    <img src="/img/book/quarkus-cookbook.png" />
  </center>
</p>
...
<p style="text-align: justify;">Quarkus also give support to Cross-Origin Resource Sharing (CORS) by use of quarkus.http.cors attribute inside of application.properties.</p>
...
<p style="text-align: justify;">The Quarkus can use applicationproperties/yaml to access the properties for the application. Those properties can be accessed by the class org.eclipse.microprofile.config.inject.ConfigProperty. The properties can be unic or a list.</p>
...
<p style="text-align: justify;">If no profile is declarad so is default values. If not, Quarkus recognize different profiles for the environment: test, dev, prod (%{profile}.config.key=value.)</p>
...
<p style="text-align: justify;">PS: Eclipse MicroProfile Configuration has some default converters as boolean and Boolean that can has, e.g, true, 1, YES, Y, and ON to TRUE and FALSE to toher cases.
...
<p style="text-align: justify;">The converter are very useful the get some data that comes from the endpoint and make it a known object.</p>
